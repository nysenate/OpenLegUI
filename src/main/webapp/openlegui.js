...
// allows you to pull parameters from the url. especially useful for GET
// requests
function getUrlVars() {
	var vars = {};
...
//gets the page hashed variables
function getGetVars() {
	var vars = {};
...
//scans through the text section and finds all blocks of text greater than 10 lines. it then surrounds those blocks with a div tag so they can be indentified to collapse
function breakUpText(text) {
	...
	// for the first block, select only the first double line break since there
	// won't be one at the beginning of the text
	end = text.indexOf("\n\n", 0);
	...
	while (beginning != -1 || end != -1) {
		...
		// if there isn't another double line break, it has reached the end of
		// the text
		if (end == -1) {
			end = text.length;
		}
	...
	for ( var i = 0; i < textArray.length; i++) {
		...
		// if the number is greater than 10, reformat it so it is now within a
		// div
		if (numBreaks > 10) {
	...
	//replace any newlines with a br tag
	text = text.replace(/\n/gi, "<br/>");
	return text;
}
// performs a search according to the provided search term. pulls the results
// and places them in the page
function showSearchResults(pageId, pageSize, term) {
	//get the path from the passed parameters
	var path = 'http://directory.nysenate.gov/legislation/2.0/search.jsonp?pageIdx='
			+ pageId + '&pageSize=' + pageSize + '&term=' + term;
	var pages = 0;
	$
			.get(
					path,
					function(data) {
						...
						$.each(data.response.results, function(i, item) {
							if (odd == 'odd')
								...
								odd = 'odd';
							//generate a url to view the documents
							if (filtered[0] == null) {
								contents += '<div id="' + odd + '">'
										+ 'Name: <a href=view' + item.otype
										+ '.jsp?oid=' + item.oid + '>'
										+ item.oid + '</a> | Type: '
										+ item.otype + '</div><br/>';
							} else {
						...
						if (data.response.metadata.totalresults == 0) {
							...
							// if the total results is the same number as the
							// page size, set the pages to 1
							if (data.response.metadata.totalresults % pageSize == 0) {
								pages = 1;
							}

							// otherwise, perform integer division to find the
							// number of pages'+i+''+i+' needed. pages++ brings
							// the number up to the correct value
							else {
								pages = ~~(data.response.metadata.totalresults / pageSize);
								pages++;
							}

							// sets up the first page link, which will always
							// display no matter the number of results
							var links = [];
							'+i+'
							urlfilter = '';
							if (filtered[0] != null) {
							...
							links = '<br/>Jump to page: <a class="pages" href=#search='
									+ term
									+ '&pageId=1'
									+ urlfilter
									+ ' onclick=' + fun + '>1</a> ';
							...
							for ( var i = 2; i < pages + 1; i++) {
								fun = 'showSearchResults(' + i + ',20,"' + term
										+ '")';
								links += '<a class="pages" href=#search='
										+ term + '&pageId=' + i + urlfilter
										+ ' onclick=' + fun + '>' + i + '</a> ';
							}
						}
						showSearchResults
